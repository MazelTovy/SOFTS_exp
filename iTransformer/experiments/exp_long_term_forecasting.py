...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def compute_mask_penalty_loss(self, outputs, batch_y, batch_x, f_dim):
        ...
        mask_output = self.model(mask_batch, None, None, None)
        penalty = nn.MSELoss()(outputs, mask_output)

        # Get penalty_factor from args or use default value
        penalty_factor = getattr(self.args, 'penalty_factor', 0.1)
        penalty_loss = penalty * penalty_factor
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        ...
                        if use_mask_penalty:
                            loss = self.compute_mask_penalty_loss(outputs, batch_y, batch_x, f_dim)
                    ...
                    if use_mask_penalty:
                        penalty_loss = self.compute_mask_penalty_loss(outputs, batch_y, batch_x, f_dim)
                        penalty_losses.append(penalty_loss.item())
                        loss = original_loss + penalty_loss
                    else:
                        loss = original_loss

                if (i + 1) % 100 == 0:
                    loss_float = original_loss.item()
                    train_loss.append(loss_float)
                    if use_mask_penalty:
                        train_penalty_losses.append(penalty_loss.item())
            ...
            train_penalty_losses = np.average(train_penalty_losses) if train_penalty_losses else 0
            
            vali_loss = self.vali(vali_data, vali_loader, criterion)
            test_loss = self.vali(test_data, test_loader, criterion)

            print("Epoch: {0}, Steps: {1} | Train Loss: {2:.7f} Penalty Loss: {3:.7f} Vali Loss: {4:.7f} Test Loss: {5:.7f}".format(
                epoch + 1, train_steps, train_loss, train_penalty_losses, vali_loss, test_loss))
