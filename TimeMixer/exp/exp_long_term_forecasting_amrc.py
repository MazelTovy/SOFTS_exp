...
class Exp_Long_Term_Forecast(Exp_Basic):
    """
    AMRC: Adaptive Mask with Representation Consistency for TimeMixer
    ...
    def compute_embedding_penalty(self, outputs, batch_y, batch_x, batch_x_mark):
        """Compute embedding-based consistency penalty loss for TimeMixer"""
        criterion = nn.MSELoss()
        ...
        # For TimeMixer, we need to extract embeddings after multi-scale processing
        # Get multi-scale processed inputs
        x_enc_list, x_mark_enc_list = model._Model__multi_scale_process_inputs(batch_x, batch_x_mark)
        ...
        if x_mark_enc_list is not None:
            for i, (x, x_mark) in enumerate(zip(x_enc_list, x_mark_enc_list)):
                B, T, N = x.size()
                x = model.normalize_layers[i](x, 'norm')
                if model.channel_independence == 1:
        ...
        # Pre-encode (decomposition)
        x_list = model.pre_enc(x_list)
        
        # Get embeddings
        enc_out_list = []
        if x_mark_enc_list is not None:
            for i, (x, x_mark) in enumerate(zip(x_list[0], x_mark_list)):
                enc_out = model.enc_embedding(x, x_mark)  # [B,T,C]
                enc_out_list.append(enc_out)
        else:
            for i, x in enumerate(x_list[0]):
                enc_out = model.enc_embedding(x, None)  # [B,T,C]
                enc_out_list.append(enc_out)
        
        # Apply the first PDM block to get initial embeddings
        if len(model.pdm_blocks) > 0:
            enc_out_list = model.pdm_blocks[0](enc_out_list)
        
        # Use the first scale embedding
        input_emb = enc_out_list[0]
        
        # Flatten embeddings for comparison
        batch_size = batch_x.size(0) if model.channel_independence == 0 else batch_x.size(0) * batch_x.size(2)
        
        # Reshape embeddings properly
        if model.channel_independence == 1:
            # Reshape back to original batch size
            _, T, C = input_emb.shape
            N = batch_x.size(2)
            input_emb = input_emb.reshape(batch_x.size(0), N, T, C)
            input_emb_flat = input_emb.reshape(batch_x.size(0), -1)
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        ...
                        if use_emb_penalty:
                            _, emb_penalty = self.compute_embedding_penalty(outputs, batch_y, batch_x, batch_x_mark)
