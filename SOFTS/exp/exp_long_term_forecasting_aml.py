...
def invert_linear(linear_layer, Y):
    # 获取权重和偏置
    W = linear_layer.weight  # 形状 [out_features, in_features]
    has_bias = linear_layer.bias is not None
    b = linear_layer.bias if has_bias else 0.0

    # 计算权重转置的伪逆
    W_t_pinv = torch.linalg.pinv(W.T)  # 形状 [out_features, in_features]

    # 减去偏置并映射回X
    X_reconstructed = (Y - b) @ W_t_pinv
